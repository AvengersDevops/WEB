	...
	stages
	{
		...
		stage("DEPLOY")
		{
			steps
			{
				...
				sh "docker-compose exec testcafe bash -c 'cd /Tests' && docker-compose exec testcafe bash -c 'export HTTP_ENV='http://localhost:5070'' && docker-compose exec testcafe bash -c './node_modules/.bin/testcafe edge:headless TestCafeTests.js -r junit:report.xml'"
