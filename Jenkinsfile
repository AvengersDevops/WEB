	...
	stages
	{
		...
		stage("DEPLOY")
		{
			steps
			{
				...
				sh "docker-compose exec --privileged -T testcafe bash -c 'cd /Tests && export HTTP_ENV='http://localhost:5070' && ./node_modules/.bin/testcafe edge:headless TestCafeTests.js -r junit:report.xml'"
